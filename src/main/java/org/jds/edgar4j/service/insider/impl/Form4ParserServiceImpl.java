...
public class Form4ParserServiceImpl implements Form4ParserService {
    ...
    private NonDerivativeTransaction parseNonDerivativeTransaction(Element element) {
        try {
            ...
            Element transactionDateElement = getFirstElementByTagName(element, "transactionDate");
            if (transactionDateElement != null) {
                transaction.setTransactionDate(getElementTextContent(transactionDateElement, "value"));
            }

            Element transactionCodingElement = getFirstElementByTagName(element, "transactionCoding");
            if (transactionCodingElement != null) {
                transaction.setTransactionCode(getElementTextContent(transactionCodingElement, "transactionCode"));
            }
            
            Element transactionAmountsElement = getFirstElementByTagName(element, "transactionAmounts");
            if (transactionAmountsElement != null) {
                transaction.setSharesTransacted(getElementTextContent(transactionAmountsElement, "transactionShares"));
                transaction.setPricePerShare(getElementTextContent(transactionAmountsElement, "transactionPricePerShare"));
                transaction.setAcquiredDisposed(getElementTextContent(transactionAmountsElement, "transactionAcquiredDisposedCode"));
            }
