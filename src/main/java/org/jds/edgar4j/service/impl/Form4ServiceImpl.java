...
public class Form4ServiceImpl implements Form4Service {
        ...
        public CompletableFuture<HttpResponse<String>> downloadForm4(String cik, String accessionNumber, String primaryDocument) {
                log.info("Download form 4 for CIK: {}, Accession: {}", cik, accessionNumber);
        ...
        public Form4 parseForm4(String html) {
                log.info("Parsing Form 4 HTML");

                try {
                        Document doc = Jsoup.parse(html);

                        Form4 form4 = Form4.builder()
                        ...
                        if (reportingOwner != null) {
                                form4.getReportingOwners().add(reportingOwner);
                        }

                        // Parse issuer information
                        parseIssuerInfo(doc, form4);

                        // Parse transaction date (period of report)
                        parsePeriodOfReport(doc, form4);

                        // Parse Table I - Non-Derivative Transactions
                        parseNonDerivativeTransactions(doc, form4);

                        // Parse Table II - Derivative Transactions
                        parseDerivativeTransactions(doc, form4);

                        // Parse footnotes and remarks
                        parseFootnotesAndRemarks(doc, form4);

                        // Parse signature
                        parseSignature(doc, form4);

                        log.info("Successfully parsed Form 4 with {} non-derivative and {} derivative transactions",
                                form4.getNonDerivativeTransactions().size(),
                                form4.getDerivativeTransactions().size());

                        return form4;
                } catch (Exception e) {
                        log.error("Error parsing Form 4: {}", e.getMessage(), e);
                        return Form4.builder().build();
                }
        }
        ...
        private void parseIssuerInfo(Document doc, Form4 form4) {
                                ...
                                if (parent != null) {
                                        ...
                                        if (parentText.contains("[") && parentText.contains("]")) {
                                                ...
                                                if (start > 0 && end > start) {
                                                        ...
                                                        form4.setTradingSymbol(ticker);
                                                        form4.setIssuerName(text);

                                                        // Extract CIK
                                                        String cik = href.substring(href.indexOf("CIK=") + 4);
                                                        form4.setIssuerCik(cik.replaceAll("[^0-9]", ""));
                                                        break;
                                                }
                                        }
                                }
        ...
        private void parsePeriodOfReport(Document doc, Form4 form4) {
        ...
        private void parseNonDerivativeTransactions(Document doc, Form4 form4) {
                // Find Table I
                Elements tables = doc.select("table");
                for (Element table : tables) {
                        if (table.text().contains("Table I") && table.text().contains("Non-Derivative")) {
                                ...
                                if (!tbody.isEmpty()) {
                                        ...
                                        for (Element row : rows) {
                                                ...
                                                if (transaction != null) {
                                                        form4.getNonDerivativeTransactions().add(transaction);
                                                }
                                        }
                                }
                                break;
        ...
        private void parseDerivativeTransactions(Document doc, Form4 form4) {
                // Find Table II
                Elements tables = doc.select("table");
                for (Element table : tables) {
                        if (table.text().contains("Table II") && table.text().contains("Derivative")) {
                                ...
                                if (!tbody.isEmpty()) {
                                        ...
                                        for (Element row : rows) {
                                                ...
                                                if (transaction != null) {
                                                        form4.getDerivativeTransactions().add(transaction);
                                                }
                                        }
                                }
                                break;
        ...
        private void parseFootnotesAndRemarks(Document doc, Form4 form4) {
                ...
                for (Element table : tables) {
                        ...
                        if (text.contains("Explanation of Responses")) {
                                ...
                                form4.setFootnotes(footnotes.toString().trim());
                        }
        ...
        private void parseSignature(Document doc, Form4 form4) {
                Elements tables = doc.select("table");
                for (Element table : tables) {
                        Elements rows = table.select("tr");
                        for (Element row : rows) {
                                        ...
                                        if (prevRow != null) {
                                                ...
                                                if (cells.size() >= 2) {
                                                        ...
                                                        if (!sigElements.isEmpty()) {
                                                                form4.setSignature(sigElements.text());
                                                        }
                                                        ...
                                                        if (cells.size() >= 3) {
                                                                ...
                                                                if (!dateElements.isEmpty()) {
                                                                        ...
                                                                        form4.setSignatureDate(sigDate);
                                                                }
                                                        }
                                                }
                                        }
