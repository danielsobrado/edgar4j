...
public class XbrlService {
    ...
    private final Cache<String, Mono<XbrlInstance>> parsedUrlCache = Caffeine.newBuilder()
            .maximumSize(64)
    ...
    public Mono<XbrlInstance> parseFromUrl(String url) {
        ...
        Mono<XbrlInstance> cached = parsedUrlCache.getIfPresent(normalizedUrl);
        if (cached != null) {
            log.debug("Using cached XBRL instance for: {}", normalizedUrl);
            return cached;
        }

        Mono<XbrlInstance> loader = buildParsedUrlMono(normalizedUrl);
        parsedUrlCache.put(normalizedUrl, loader);
        return loader;
    }

    private Mono<XbrlInstance> buildParsedUrlMono(String url) {
        log.info("Fetching XBRL from: {}", url);

        return loadUrlBytesWithPersistentCache(url, 50 * 1024 * 1024)
                ...
                .doOnError(error -> parsedUrlCache.invalidate(url))
                .cache();
    }
    ...
    public Mono<XbrlPackageHandler.PackageResult> parsePackageFromUrl(String url) {
        ...
        return loadUrlBytesWithPersistentCache(url, 100 * 1024 * 1024)
    ...
    public Map<String, Long> getCacheStats() {
        Map<String, Long> stats = new LinkedHashMap<>(taxonomyResolver.getCacheStats());
        stats.put("parsedInstances", parsedUrlCache.estimatedSize());
        return stats;
    }
    ...
    private Mono<byte[]> loadUrlBytesWithPersistentCache(String url, int maxInMemorySize) {
        return Mono.fromCallable(() -> downloadedResourceStore.readBytes("xbrl", url))
                .subscribeOn(Schedulers.boundedElastic())
                .flatMap(cached -> {
                    if (cached.isPresent()) {
                        log.debug("Using cached XBRL source for {}", url);
                    ...
                    return webClient.get()
                            ...
                            .flatMap(content -> Mono.fromCallable(() -> {
                                        var cachePath = downloadedResourceStore.writeBytes("xbrl", url, content);
                                        log.debug("Cached XBRL source for {} at {}", url, cachePath);
                                        return content;
                                    })
                                    .subscribeOn(Schedulers.boundedElastic()));
                });
