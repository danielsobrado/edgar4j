...
public class ErrorRecoveryParser {
    ...
    public ParsedDocument parse(byte[] content, String contentType) {
        ...
        try {
            switch (docType) {
                ...
                case INLINE_XBRL:
                case HTML:
                    return parseHtml(text, docType);
                default:
                    // Try HTML parser as fallback (most lenient)
                    return parseHtml(text, DocumentType.HTML);
            }
        } catch (Exception e) {
     ...
     */
    private ParsedDocument parseHtml(String content, DocumentType documentType) {
        Document doc = Jsoup.parse(content, "", Parser.htmlParser());
        return new ParsedDocument(doc, documentType, false);
    }
