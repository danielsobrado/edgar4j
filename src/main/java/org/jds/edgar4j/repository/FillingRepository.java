...
public interface FillingRepository extends MongoRepository<Filling, String> {
    ...
    @Query("{ 'cik': ?0, 'formType.number': { $in: ?1 } }")
    Page<Filling> findByCikAndFormTypeIn(String cik, List<String> formTypeNumbers, Pageable pageable);

    @Query("{ 'cik': ?0, 'formType.number': ?1, 'fillingDate': { $gte: ?2, $lte: ?3 } }")
    Page<Filling> findByCikAndFormTypeAndFillingDateBetween(String cik, String formTypeNumber, Date startDate, Date endDate, Pageable pageable);

    @Query("{ 'cik': { $in: ?0 } }")
    Page<Filling> findByCikIn(List<String> ciks, Pageable pageable);

    @Query("{ 'cik': { $in: ?0 }, 'formType.number': ?1 }")
    Page<Filling> findByCikInAndFormType(List<String> ciks, String formTypeNumber, Pageable pageable);

    @Query("{ 'cik': { $in: ?0 }, 'formType.number': { $in: ?1 } }")
