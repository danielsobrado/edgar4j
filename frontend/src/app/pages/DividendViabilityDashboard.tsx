...
import {
  ...
  type DividendMetricDefinition,
  type DividendRating,
  type DividendScreen,
  type DividendScreenRequest,
  type DividendHistoryTrendDirection,
...
function formatFiscalYearEnd(value?: string | number | null): string {
  if (value == null) {
    return '-';
  }
...
function formatPercent(value: number | null, maximumFractionDigits: number = 1): string {
  if (value == null) {
    return '-';
  }
...
function formatMultiple(value: number | null, maximumFractionDigits: number = 2): string {
  if (value == null) {
    return '-';
  }
...
function severityClasses(severity: DividendAlert['severity']): string {
  if (severity === 'HIGH') {
...
function ratingClasses(rating: DividendOverview['viability']['rating']): string {
  if (rating === 'SAFE') {
...
function historyTrendClasses(trend: DividendHistoryTrendDirection): string {
  if (trend === 'UP') {
...
function historyTrendLabel(trend: DividendHistoryTrendDirection): string {
...
function eventTypeLabel(eventType: DividendEvent['eventType']): string {
  return eventType.replaceAll('_', ' ');
}

function eventTypeClasses(eventType: DividendEvent['eventType']): string {
...
function formatEvidenceTitle(filing?: DividendFilingEvidence['filing'] | null): string {
  if (!filing) {
        ...
        <>
                    ...
                    <div className="flex shrink-0 flex-col items-start gap-2 lg:items-end">
                      ...
                      {event.accessionNumber && (
                        <Button
                          ...
                          variant="outline"
                          size="sm"
                          onClick={() => openEvidence(event.accessionNumber)}
                          className="rounded-full"
                        >
                          View evidence
                        </Button>
          ...
          <section className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-sm">
            <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
              ...
              <div className="flex flex-wrap gap-2">
                {DEFAULT_COMPARE_METRICS.map((metric) => (
                  <span key={metric} className="rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-xs font-medium uppercase tracking-[0.12em] text-slate-600">
                    {metric.replaceAll('_', ' ')}
                  </span>
                ))}
            ...
            <form onSubmit={runComparison} className="mt-5 flex flex-col gap-3 lg:flex-row lg:items-center">
            ...
            <div className="mt-5">
              {comparisonState.status === 'idle' && (
                <div className="rounded-3xl border border-slate-200 bg-slate-50 px-4 py-4 text-sm text-slate-600">
                  Add one or more peer tickers to compare against {company.ticker || company.cik}.
                </div>
              )}
          ...
          <section className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-sm">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <span className="rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-sm text-slate-600">
                  {ANNUAL_TABLE_COLUMNS.length} tracked columns
                </span>
              </div>
            </div>
              ...
              <table className="min-w-full divide-y divide-slate-200">
                <thead className="bg-slate-50">
                  <tr>
                    ...
                    {ANNUAL_TABLE_COLUMNS.map((column) => (
                ...
                <tbody className="divide-y divide-slate-200 bg-white">
                  {historyRows.length === 0 ? (
                    <tr>
                      <td colSpan={ANNUAL_TABLE_COLUMNS.length + 1} className="px-4 py-6 text-sm text-slate-600">
