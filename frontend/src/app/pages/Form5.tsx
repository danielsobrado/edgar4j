...
import { Search, FileCheck, Calendar, Building2, ExternalLink, RefreshCw, User, Briefcase, TrendingUp, TrendingDown } from 'lucide-react';
import { useRecentForm5, useForm5Search } from '../hooks';
import { LoadingSpinner } from '../components/common/LoadingSpinner';
import { ErrorMessage } from '../components/common/ErrorMessage';
import { EmptyState } from '../components/common/EmptyState';
import { Pagination } from '../components/common/Pagination';
import { Form5 as Form5Type, Form4Transaction } from '../api/types';

function getOwnerTypeBadge(form: Form5Type) {
...
function OwnerTypeBadge({ form }: { form: Form5Type }) {
...
function getTransactionCodeDescription(code: string | undefined): string {
  ...
  return descriptions[code.toUpperCase()] || `Code: ${code}`;
}

function FilingCard({ filing, onClick }: { filing: Form5Type; onClick: () => void }) {
  const transactionCount = filing.transactions?.length || 0;
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between mb-3">
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="px-2 py-0.5 bg-indigo-100 text-indigo-700 rounded text-xs">Form 5</span>
          </div>
      ...
      <div className="grid grid-cols-3 gap-2 text-sm mb-3">
        ...
        <div>
          <p className="text-gray-500">Txns</p>
...
function TransactionsTable({ transactions }: { transactions: Form4Transaction[] }) {
  if (!transactions || transactions.length === 0) {
    return <p className="text-gray-500 text-sm">No transactions reported.</p>;
  }
      ...
      <table className="min-w-full divide-y divide-gray-200">
        <thead className="bg-gray-50">
          <tr>
            ...
            <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">A/D</th>
          </tr>
        </thead>
        <tbody className="bg-white divide-y divide-gray-200">
          {transactions.map((txn, idx) => (
            <tr key={idx} className="hover:bg-gray-50">
              <td className="px-4 py-3 text-sm font-medium text-gray-900">{txn.securityTitle}</td>
              <td className="px-4 py-3 text-sm text-gray-600">{txn.transactionDate || '-'}</td>
              <td className="px-4 py-3 text-sm">
                <span className="px-2 py-1 bg-gray-100 rounded text-xs" title={getTransactionCodeDescription(txn.transactionCode)}>
                  {txn.transactionCode || '-'}
                </span>
              </td>
              <td className="px-4 py-3 text-sm text-right">{formatNumber(txn.transactionShares)}</td>
              <td className="px-4 py-3 text-sm text-right">{formatCurrency(txn.transactionPricePerShare)}</td>
              <td className="px-4 py-3 text-sm">
                {txn.acquiredDisposedCode === 'A' ? (
                  <span className="flex items-center gap-1 text-green-600">
                    <TrendingUp className="w-4 h-4" />
                    Acquired
                  </span>
                ) : txn.acquiredDisposedCode === 'D' ? (
    ...
    <div className="overflow-x-auto">
      <table className="min-w-full divide-y divide-gray-200">
        <thead className="bg-gray-50">
          <tr>
            <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Security</th>
            <th className="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase">Shares Owned</th>
...
function FilingDetail({ filing, onClose }: { filing: Form5Type; onClose: () => void }) {
  const [activeTab, setActiveTab] = React.useState<'transactions' | 'holdings'>('transactions');
  const secDocumentUrl = `https://www.sec.gov/cgi-bin/browse-edgar?action=getcompany&CIK=${filing.cik}&type=5&dateb=&owner=include&count=40`;
        ...
        <div className="flex items-start justify-between">
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <span className="px-2 py-1 bg-indigo-100 text-indigo-700 rounded text-sm">Form 5</span>
            </div>
            <p className="text-gray-500 text-sm">
              Annual Statement of Beneficial Ownership | Filed: {filing.filedDate}
            </p>
          </div>
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          ...
          <div>
            ...
            <p>{filing.documentType || 'Form 5'}</p>
          </div>
        </div>
        ...
        <div>
          <div className="border-b border-gray-200 mb-4">
            <nav className="flex gap-4">
              ...
              >
                Transactions ({filing.transactions?.length || 0})
              </button>
              <button
              ...
              >
                Holdings ({filing.holdings?.length || 0})
              </button>
            </nav>
          </div>

          {activeTab === 'transactions' ? (
            <TransactionsTable transactions={filing.transactions || []} />
          ) : (
...
export function Form5Page() {
  ...
  const [selectedFiling, setSelectedFiling] = React.useState<Form5Type | null>(null);

  const { filings: recentFilings, loading: recentLoading, error: recentError, refresh } = useRecentForm5(12);
  const {
      ...
      <div className="bg-white rounded-lg shadow-sm p-6">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-semibold">Form 5 - Annual Ownership</h1>
              <p className="text-gray-500 text-sm">Annual statement of changes in beneficial ownership</p>
            </div>
          </div>
          <button
            onClick={refresh}
      ...
      {!selectedFiling && (
        <div className="bg-white rounded-lg shadow-sm p-6">
          <h2 className="font-semibold mb-4">
            {hasSearched ? `Search Results (${totalElements.toLocaleString()} filings)` : 'Recent Form 5 Filings'}
          </h2>
