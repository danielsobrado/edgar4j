...
export function XbrlAnalysisPanel({ filingUrl, isXbrl, isInlineXbrl }: XbrlAnalysisPanelProps) {
  ...
  const { data: statements, loading: statementsLoading, error: statementsError, load: loadStatements } = useFinancialStatements();
  const { facts, loading: factsLoading, error: factsError, loadFacts, searchFacts } = useXbrlFacts();

  // Load analysis only when the overview tab is active.
  useEffect(() => {
    if (expanded && activeTab === 'overview' && !analysis && !analysisLoading && !analysisError && (isXbrl || isInlineXbrl)) {
      analyze(filingUrl).catch(console.error);
    }
  }, [expanded, activeTab, analysis, analysisLoading, analysisError, isXbrl, isInlineXbrl, filingUrl, analyze]);

  // Load statements when the statements tab is active.
  useEffect(() => {
    if (expanded && activeTab === 'statements' && !statements && !statementsLoading && !statementsError) {
      loadStatements(filingUrl).catch(console.error);
    }
  }, [expanded, activeTab, statements, statementsLoading, statementsError, filingUrl, loadStatements]);

  // Load facts when the facts tab is active.
  useEffect(() => {
    if (expanded && activeTab === 'facts' && facts.length === 0 && !factsLoading && !factsError) {
      loadFacts(filingUrl).catch(console.error);
    }
  }, [expanded, activeTab, facts.length, factsLoading, factsError, filingUrl, loadFacts]);
          ...
          <div className="p-6">
            {activeTab === 'overview' && (
              <OverviewTab
                ...
                error={analysisError}
                onRetry={() => analyze(filingUrl).catch(console.error)}
              />
            )}
            {activeTab === 'statements' && (
              <StatementsTab
                ...
                onRetry={() => loadStatements(filingUrl).catch(console.error)}
              />
            )}
            {activeTab === 'facts' && (
              <FactsTab
                ...
                onRetry={() => loadFacts(filingUrl).catch(console.error)}
              />
            )}
          </div>
        </div>
    ...
    <div className="space-y-4">
      ...
      <div className="overflow-x-auto border border-gray-200 rounded-lg">
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="divide-y divide-gray-100">
            {factRows.slice(0, 100).map((fact, index) => (
      ...
      {factRows.length > 100 && (
        <p className="text-sm text-gray-500 text-center">
          Showing first 100 of {factRows.length} facts
        </p>
      )}
